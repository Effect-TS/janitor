...
for (const existing of [false, true, "fork"])
  test(`controlled publication ${existing ? "updates the existing blog PR" : "creates the designated branch"} and incorporates concurrent human commits`, async () => {
    ...
    writeFileSync(join(human, "README.md"), "historical content\n")
    git(human, "add", ".")
    git(human, "commit", "-m", "history excluded from initial checkout")
    writeFileSync(join(human, "README.md"), "base\n")
    git(human, "add", ".")
    git(human, "commit", "-m", "base")
    git(human, "push", "origin", "main")
    const branch = existing ? "blog/post" : `janitor/${"a".repeat(64)}`
    if (existing) {
      git(human, "checkout", "-b", branch)
      writeFileSync(join(human, "post.md"), "A shared blog post\n")
      git(human, "add", ".")
      git(human, "commit", "-m", "Draft blog post")
      git(human, "push", "origin", branch)
    }
    const initialHead = git(human, "rev-parse", "HEAD")
    if (existing === "fork") {
      git(
        root,
        "--git-dir=" + origin + "/owner/repo.git",
        "update-ref",
        "refs/pull/7/head",
        initialHead,
      )
      git(human, "push", "origin", "--delete", branch)
    }
    ...
    try {
      await call("/clone", {
        ...
        ...(existing ? { branch, ...(existing === "fork" ? { pullRequestNumber: 7 } : {}) } : {}),
      })
      ...
      if (existing === "fork") git(human, "push", "origin", branch)
      if (existing) {
        assert.equal(git(work, "branch", "--show-current"), branch)
        assert.equal(readFileSync(join(work, "post.md"), "utf8"), "A shared blog post\n")
      }
      ...
      assert.equal(prepared.remoteHead, existing ? initialHead : null)
      ...
      if (existing) git(human, "pull", "--ff-only", "origin", branch)
      else git(human, "checkout", "-b", input.branch, `origin/${input.branch}`)
