...
3. OpenCode runs in the object; tools call the owned sandbox. A first attempt starts the model while the checkout is still being prepared; every tool waits for the checkout before touching it. A retry prepares first, because the model must be told when the workspace was restored. Commands default to ten minutes and turns to thirty (`JANITOR_COMMAND_TIMEOUT_MS`, `JANITOR_TURN_TIMEOUT_MS`).
...
Slack keeps an immediate receipt acknowledgement and delivers assistant messages as conversation replies. Routine runner stages and completion do not generate Slack status messages. Interruptions still explain what happened and offer Retry or Skip. Before each turn, the host refreshes its conversation instructions, including for existing sessions. Quick requests get direct answers in the requested format; longer work can include useful progress. Repository selection stays in the agent's context without requiring an announcement.
