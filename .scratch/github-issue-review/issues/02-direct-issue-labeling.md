...
- [x] Automatic issue labeling and issue-labeling previews acquire the required facts directly from GitHub; cache freshness, synchronization generations, and synchronization success do not admit or block them.
- [x] Preserve policy references, gate policies, AI inputs/cache rules, grouping, priority, label ownership, and unknown/failed evaluation behavior. Closed issues remain outside labeling scope.
- [x] Replace synchronization-derived admission and stale-work qualification with automation-owned identities and current repository/configuration checks for the migrated issue path.
- [x] Each actual label-write attempt obtains the needed current remote state and checks repository authority and current rule configuration. A cached authorization result cannot authorize a later write.
- [x] Respect GitHub throttling and existing bounded retry behavior; unavailable facts or operational failure must not masquerade as a non-match.
- [x] Labeling previews show current evidence, result, and proposed effect without label writes or dependence on cache readiness.
- [x] Retire or fence pending legacy issue jobs during cutover while leaving PR labeling on its existing safe path until ticket 03. New sync lifecycle events cannot erase or improperly release direct issue work.
- [x] Verify equivalent supported issue-policy outcomes plus stale UI data, sync failure, out-of-order events, rule changes, and pause/access races.
