...
## Sandbox runner redesign (2026-09-14)

The [consolidated spec](spec.md) supersedes the SQLite-only design above. The runner now extends the Sandbox SDK object: one container per session, OpenCode in the object with its SQLite conversation, and focused Effect services for admission and attempt identity (`SessionCoordinator`), workspace preparation (`RepositoryCheckout`), recovery points (`RecoveryStore`), sandbox Git publication (`WorkspacePublication`) and the sandbox-backed tools (`SandboxTools`). The native suspended-session sweep is replaced with an inert layer, so a restart never restarts a model turn; interrupted attempts wait for a teammate's Retry or Skip delivered through Slack buttons and a new interactivity route.

Retired: the SQLite workspace store and GitHub tree/blob access, the legacy archive importer, compatibility and release-manifest machinery, the maintenance barrier and hold (service, routes, tables, token, CI smoke), the Miniflare harness and the acceptance driver. Protocol version 3 carries `turn.*` events and Retry/Skip actions. Migration `0032_sandbox_runner.sql` performs the cutover: existing sessions end with cleanup tombstones, their handoffs and Slack threads are dropped.

Default PR checks are `vp check`, the runner typecheck and bundle, and `vp test`; the local Alchemy smoke is opt-in. Production needs two new secrets, `JANITOR_SANDBOX_R2_ACCESS_KEY_ID` and `JANITOR_SANDBOX_R2_SECRET_ACCESS_KEY`, because production backups upload through presigned R2 URLs. Real-container behavior (backup consistency, overlay restore, the `runuser` isolation) was not exercised in this worktree; the opt-in local smoke and the first deployment are where that evidence comes from.
