...
2026-09-13: Changed the current deployment candidate and bounded smoke driver to OpenRouter at the user's request. Uses the pinned SDK's native OpenRouter adapter and an immutable upstream selection, with provider fallback disabled. The original direct-Groq record remains unchanged. The runtime binding name stays `JANITOR_AGENT_RUNNER_MODEL_API_KEY`, now holding an OpenRouter key. Live validation remains pending; earlier Groq-specific setup and authorization instructions are superseded by `runner/MODEL-VALIDATION.md`.

OpenRouter follow-up validation: controlled driver failed before native adapter support and passes after it. Focused model suites passed with 11 passed and one live skip; full runner suite passed with 61 passed and two skipped. Runner typechecking, production build and root `vp check --fix` passed. Standards and spec reviews passed. No live provider calls or deployment were performed.
